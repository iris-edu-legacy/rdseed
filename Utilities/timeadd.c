/*===========================================================================*/
/* SEED reader     |              timeadd                  |         utility */
/*===========================================================================*/
/*
	Name:	timeadd
	Purpose:add some number of seconds to a time of the form yyyyddd
		hh:mm:ss.ffff
	Usage:	struct time timeadd ();
		struct time time;
		struct time newtime;
		int increment;
		newtime = timeadd (time, increment);
	...
	CL - changed increment to unsigned
...
struct time timeadd (time, increment)
struct time time;
unsigned int increment;
{
	...
	timeinc.fracsec = increment % 10000;
	increment = increment / 10000;
	timeinc.hour = increment / 3600;
	timeinc.minute = (increment - (timeinc.hour * 3600)) / 60;
	timeinc.second = increment - (timeinc.hour * 3600) - (timeinc.minute * 60);
...
struct time timesub(time, decrement)
struct time time;
unsigned int decrement;
{
	struct time newtime;			/* result */
	struct time timedec;			/* time increment */

	/* initialize year and day */
	newtime.year = time.year;
	newtime.day = time.day;

	/* convert decrement to hh:mm:ss.ffff */
	timedec.fracsec = decrement % 10000;
	decrement = decrement / 10000;
	timedec.hour = decrement / 3600;
	timedec.minute = (decrement - (timedec.hour * 3600)) / 60;
	timedec.second = decrement - (timedec.hour * 3600) - (timedec.minute * 60);

	/* sub the decrement to the original time */
	newtime.hour = time.hour - timedec.hour;
	newtime.minute = time.minute - timedec.minute;
	newtime.second = time.second - timedec.second;
	newtime.fracsec = time.fracsec - timedec.fracsec;

	/* adjust fractional seconds, decrement seconds if necessary */
	if (newtime.fracsec < 0)
	{
		newtime.second--;
		newtime.fracsec += 10000;
	}

	/* adjust seconds, decrements minutes if necessary */
	if (newtime.second < 0)
	{
		newtime.minute--;
		newtime.second += 60;
	...
	/* adjust minutes, decrement hours if necessary */
	if (newtime.minute < 0)
	{
		newtime.hour--;
		newtime.minute += 60;
	...
	/* adjust hours, decrement days if necessary */
	if (newtime.hour < 0)
	{
		newtime.day--;
		newtime.hour += 24;
	...
	/* decrement year if necessary, allowing for leap year */
	if (newtime.day <= 0)
	{
		newtime.year--;
	
		if (isaleap(newtime.year))
			newtime.day = 366;
		else
			newtime.day = 365;
	}
